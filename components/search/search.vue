<template name="search">
	<view>
		<view class="search">
			<template>
				<u-search placeholder="输入最少6位业务编号" v-model="searchText" @custom="searchStart" focus
					@search="searchStart"></u-search>
			</template>
		</view>
		<view :class="'wanted-' + theme" v-if="showWant">
			<view class="header">指定搜索方式</view>
			<view class="list">
				<view v-for="(item,index) in hotList" :key="index" :class="[hotIndex===index ? 'bgc' :'']"
					@click="wayIndex(index)">{{item}}</view>
			</view>
		</view>
		<view :class="'wanted-' + theme" v-if="searchText && searchText.length>0">
			<view class="header">搜索结果</view>
			<view class="box">
				<view class="item" v-for="(item,index) in orderList" :key="index" @click.stop="clickDetail(item)">
		...
		<view :class="'s-' + theme" v-else-if="hList.length > 0 && searchText.length===0">
			<view class="header">
				历史记录
			...
			<view class="list">
				<uni-list :border="true">
					<uni-list-item v-for="(item,index) in hList" :key="index" @click="keywordsClick(item)" :title="item"
						clickable />
				</uni-list>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			keywordsClick(item, index) { //关键词搜索与历史搜索
